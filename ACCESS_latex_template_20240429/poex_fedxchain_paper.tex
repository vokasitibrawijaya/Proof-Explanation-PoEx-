...
\STATE Initialize reference $\mathbf{\Phi}_{ref}$ from trusted baseline (see Section~\ref{sec:ref_init})
...
\begin{enumerate}
    ...
    \item \textbf{Adaptive Attack:} Adversary with knowledge of defense threshold $\tau$ crafts updates to maximize damage while staying below detection, following~\cite{cao2024comprehensive}. Algorithm~\ref{alg:adaptive} presents the formal procedure.
\end{enumerate}

\begin{algorithm}[t]
\caption{Adaptive Attack Against PoEx}
\label{alg:adaptive}
\begin{algorithmic}[1]
\REQUIRE Threshold $\tau$, honest update $\Delta \mathbf{w}^{honest}$, attack scale $s \in [1, 5]$
\ENSURE Adversarial update $\Delta \mathbf{w}^{adv}$

\STATE \textbf{Phase 1: Reference Estimation}
\STATE Observe accepted updates from previous rounds
\STATE Estimate reference: $\hat{\mathbf{\Phi}}_{ref} \leftarrow \text{median}(\{\mathbf{\Phi}_j : j \in \mathcal{A}_{prev}\})$

\STATE \textbf{Phase 2: Constrained Attack Generation}
\STATE Initialize: $\Delta \mathbf{w}^{adv} \leftarrow -s \cdot \Delta \mathbf{w}^{honest}$ \COMMENT{Sign-flip with scale}
\STATE Compute: $\mathbf{\Phi}^{adv} \leftarrow \text{SHAP}(f_{local}^{adv})$
\STATE Compute: $\text{NSDS}_{adv} \leftarrow \text{NSDS}(\mathbf{\Phi}^{adv}, \hat{\mathbf{\Phi}}_{ref})$

\STATE \textbf{Phase 3: Adaptive Scaling}
\WHILE{$\text{NSDS}_{adv} \geq \tau - 0.05$}
    \STATE $s \leftarrow s \cdot 0.8$ \COMMENT{Reduce attack magnitude}
    \STATE $\Delta \mathbf{w}^{adv} \leftarrow -s \cdot \Delta \mathbf{w}^{honest}$
    \STATE Recompute $\text{NSDS}_{adv}$
\ENDWHILE

\STATE \textbf{Phase 4: Feedback Loop}
\IF{update was accepted in previous round}
    \STATE $s \leftarrow \min(s \cdot 1.1, 5)$ \COMMENT{Increase attack}
\ELSE
    \STATE $s \leftarrow s \cdot 0.9$ \COMMENT{Decrease attack}
\ENDIF

\RETURN $\Delta \mathbf{w}^{adv}$
\end{algorithmic}
\end{algorithm}
...
\begin{theorem}[PoEx Byzantine Resilience Bound]
...
\textit{Step 3 (Threshold-Tolerance Relationship):} For the aggregation to remain robust, we require honest contributions to dominate: $n - f > f \cdot c(\tau)$ where $c(\tau) = \tau/(1-\tau)$ captures the detection efficiency. Rearranging yields $f < n\tau/(1+\tau)$.
\end{proof}
...
\begin{figure}[t!]
    \centering
    \includegraphics[width=\columnwidth]{nsds_distribution.png}
...
\begin{figure}[t!]
    \centering
    \includegraphics[width=\columnwidth]{tpr_fpr_curve.png}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_ieee/moko.png}}]{Rachmad Andri Atmoko}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_ieee/fauzan.png}}]{M. Fauzan Edy Purnomo}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_ieee/mahdin.png}}]{Mahdin Rohmatillah}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_ieee/cries.png}}]{Cries Avian}
